...
export function draggable(node: HTMLElement, options: DraggableOptions | any) {
    let state = typeof options === 'object' && 'enabled' in options 
        ? options 
    ...
    function handle_touchmove(e: TouchEvent) {
        ...
        const dropzoneBelow = elementsBelow.find(el => 
            el.hasAttribute('data-dropzone') && el !== node
    ...
    return {
        update(newOptions: DraggableOptions | any) {
            state = typeof newOptions === 'object' && 'enabled' in newOptions 
                ? newOptions 
